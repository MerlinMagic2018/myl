...
function name_directory_yesno($var)
{
    if(! empty($var))
    {
        return __('Yes', 'myl-glossary');
    }

    return __('No', 'myl-glossary');
}
 ...
 * @return string
 */
function name_directory_make_plugin_url($index = 'glossary-item', $exclude = null)
{
    $url = array();
...
function name_directory_notify_admin_of_new_submission($directory, $input)
{
    ...
    wp_mail($admin_email,
        __('New submission for Name Directory', 'myl-glossary'),
        __('Howdy,', 'myl-glossary') . "\n\n" .
        sprintf(__('There was a new submission to the Name Directory on %s at %s', 'myl-glossary'), get_option('blogname'), get_option('home')) . "\n\n" .
        sprintf("%s: %s", __('Name', 'myl-glossary'), $input['name_directory_name']) . "\n" .
        sprintf("%s: %s", __('Description', 'myl-glossary'), $input['name_directory_description']) . "\n" .
        sprintf("%s: %s", __('Submitted by', 'myl-glossary'), $input['name_directory_submitter']) . "\n\n" .
        __('This new submission does not have the published status.', 'myl-glossary') . ' ' .
        __('Please login to your WordPress admin to review and accept the submission.', 'myl-glossary') . "\n\n" .
        sprintf("Link: %s/wp-admin/admin.php?page=myl-glossary&sub=manage-directory&dir=%d&status=unpublished", get_option('home'), $directory) . "\n\n" .
...
function name_directory_render_admin_overview_table_headerfooter()
{
    echo '<tr>
            ...
            <th width="52%" scope="col" id="title" class="manage-column column-title sortable desc">
                <span>' . __('Title', 'myl-glossary') . '</span>
            </th>
            <th width="13%" scope="col">' . __('Entries', 'myl-glossary') . '</th>
            <th width="13%" scope="col">' . __('Published', 'myl-glossary') . '</th>
            <th width="13%" scope="col">' . __('Unpublished', 'myl-glossary') . '</th>
        </tr>';
}
...
{
    echo '<div class="notice notice-info is-dismissible"><p><strong>' . __('Name Directory has been updated.', 'myl-glossary') . '</strong></p>
          <p>' . __('You can now find Name Directory directly in your Admin menu!', 'myl-glossary') . '</p></div>';
}
