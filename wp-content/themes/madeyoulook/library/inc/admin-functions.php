...
function hide_unnecessary_wordpress_menus(){
    ...
    if(!$current_user->user_login == 'dylan'){
...
/*
Description: Restores the ability to upload non-image files in WordPress 4.7.1 and 4.7.2. Please remove the plugin once WordPress 4.7.3 is available! ** Seems to be affecting WP 5.0, which is why it's included now.
...
// add_filter( 'manage_edit-product_columns', 'show_product_attributes',15 );
// function show_product_attributes($columns){

//    //remove column
//    //unset( $columns['tags'] );

//    //add column
//    $columns['pa_occasions'] = __( 'Occasions'); 
...
// function add_product_column( $columns, $postid ) {
//     if ( $columns == 'pa_occasions' ) {
//         echo get_post_meta( $postid, 'pa_occasions', true );
//     }
// }
